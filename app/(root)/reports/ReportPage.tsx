...
import { ArrowUpRight, ArrowDownRight, Download, MoreHorizontal } from "lucide-react";
import { Bar, Line, Pie } from "react-chartjs-2";
import {
  Chart as ChartJS,
  CategoryScale,
...
const ReportPage = () => {
  const { data: userStats, isLoading: userLoading } = useUserAnalytics();
  const { data: revenueStats, isLoading: revenueLoading } = useRevenueAnalytics();
  const { data: serviceStats, isLoading: serviceLoading } = useGetServiceAnalysis();
  ...
  const stats = [
    ...
    },
    {
      ...
      value: userStats?.new_registrations ?? 0,
      change: "",
      isPositive: true,
    },
    {
      icon: <span className="text-purple-500">💰</span>,
      title: "Total Revenue",
      value: `₦${revenueStats?.total_revenue?.toLocaleString() ?? 0}`,
      change: "",
      isPositive: true,
    },
    {
      icon: <span className="text-purple-500">💳</span>,
      title: "Transactions",
      value: revenueStats?.transactions_count ?? 0,
      change: "",
      isPositive: true,
    },
  ];
  ...
  const jobPerformanceData = useMemo(() => ({
    labels: serviceStats?.topProviders?.map((x: any) => x.category) ?? [],
    datasets: [
      {
        label: "Jobs Completed",
        data: serviceStats?.topProviders?.map((x: any) => x.completed_jobs) ?? [],
  ...
  const revenueFlowData = useMemo(() => ({
    labels: revenueStats?.monthly_trend?.map((x) => x.month) ?? [],
    datasets: [
      {
        label: "Revenue",
        data: revenueStats?.monthly_trend?.map((x) => x.revenue) ?? [],
        borderColor: "#FF4500",
        backgroundColor: "rgba(255, 69, 0, 0.2)",
        tension: 0.4,
        fill: true,
  ...
  const disputeSettledData = {
    ...
    datasets: [
      {
        data: [
          serviceStats?.disputeStats?.settled ?? 0,
          serviceStats?.disputeStats?.unsettled ?? 0,
        ],
        backgroundColor: ["#4B0082", "#FF4500"],
        borderWidth: 1,
      },
  ...
  const tableData =
    serviceStats?.topProviders?.map((p: any, index: number) => ({
      id: p.id,
      rank: index + 1,
  ...
  const exportToCSV = () => {
    ...
    const rows = tableData.map((item : any) =>
      tableColumns.map((col) => String(item[col.key] ?? "")).join(",")
    );
    const csvContent = [headers, ...rows].join("\n");
    const blob = new Blob([csvContent], { type: "text/csv" });
    const url = window.URL.createObjectURL(blob);
  ...
  if (userLoading || revenueLoading || serviceLoading) {
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex items-center space-x-4">
              <button className="flex items-center px-4 py-2 border rounded-lg">
                <Image src="/assets/icons/filter.png" alt="Filter" width={18} height={18} className="mr-2" />
                All filters
              </button>

              <button onClick={exportToCSV} className="flex items-center gap-2 px-4 py-2 border rounded-lg">
                <Download className="h-5 w-5" />
                Export CSV
              </button>
