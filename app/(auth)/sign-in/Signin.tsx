...
const Signin = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // ✅ Properly map API user → AuthUser
      login(
        {
          email: apiUser.email,
          ...
          kycVerificationStatus: apiUser.kycVerificationStatus,
          status: apiUser.status,
          role: apiUser.adminProfile?.role, // Role2 (matches AuthUser now)
          permissions: apiUser.adminProfile?.permissions || [],
        },
        response.data.token
      );
      ...
      // Error already handled in reactQuery hook
    }
  };
      ...
      <div className="flex flex-row items-center justify-center lg:flex-row h-screen">
        ...
        <div className="w-full max-w-[450px] lg:max-w-full lg:w-1/2 flex lg:p-8 bg-[#FFFDFA] transition-colors duration-300 mt-0 md:mt-5">
          <div className="w-full mx-6 my-12 p-0 md:p-8 rounded-2xl bg-white">
            ...
            <h2 className="text-[20px] ml-10 md:ml-0 md:text-[28px] font-semibold text-[#191926] mb-4 md:mb-0 md:my-4">
              Sign in to find trusted experts
            </h2>

            {/* Social Buttons */}
            <div className="flex flex-col ml-15 md:ml-0 w-[250px] md:w-[450px] sm:flex-row gap-3 mb-6 mt-2">
              <button className="cursor-pointer flex-1 flex items-center text-[14px] font-semibold justify-center gap-2 bg-white text-[#4B4B56] border border-gray-100 rounded-[32px] py-3 transition-colors">
                <Image
                  src="/assets/icons/apple-icon.png"
                  alt="Apple"
                  width={20}
              ...
              <button className="cursor-pointer w-full flex-1 flex items-center text-[14px] font-semibold justify-center gap-2 bg-[#2167FF] text-white border border-gray-300 rounded-[32px] py-3 transition-colors">
                <Image
                  src="/assets/icons/google-icon.png"
                  alt="Google"
                  width={26}
            ...
            <div className="relative my-4">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-2 text-[16px] bg-[#FFFDFA] text-gray-500">
                  or
                </span>
              </div>
            </div>
